...
func add_gen_level_button():
	var gen_level_button: Button = GenLevelButton.instantiate()
	
	gen_level_button.button_down.connect(generate_map)
...
func add_npcs():
	...
	var a_star: CharacterBody3D= AStar.instantiate()
	#var npc_size = Vector3(0, a_star.size.y/2-1, 0)
	a_star.transform.origin = npc_start_position# + npc_size
	add_child(a_star)
	a_star.owner = self
